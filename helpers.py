...
def load_data(pos_file, neg_file, sample_size):
    ...
    x_text = [tokenize(sent) for sent in x_text]
...
def pad_sen(sentences, sequence_length, padding_word="UNK"):
...
def build_vocabulary(vocab_file, sentences):
...
def load_data(vocab_file, pos_file, neg_file, sequence_length, sample_size=0):
    ...
    sentences, labels = load_data(pos_file, neg_file, sample_size)
    sentences_padded = pad_sen(sentences, sequence_length)

    word2id, id2word = build_vocabulary(vocab_file, sentences)
